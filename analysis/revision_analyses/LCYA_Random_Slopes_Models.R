...
for (task_name in TASKS) {
  ...
  # ── 1. GLMM: Accuracy ────────────────────────────────────────────────────
  # CDT: iscorr is unreliable in pupillometry data; use resp1_diff (hit rate)
  # as the accuracy outcome for CDT only. ADT/VDT use iscorr as normal.
  cat("\n--- GLMM: Accuracy ---\n")
  acc_outcome <- if (task_name == "CDT") "resp1_diff" else "iscorr"
  acc_beh <- if (task_name == "CDT") {
    beh %>% filter(!is.na(resp1_diff))
  } else {
    beh
  }
  r_acc <- fit_reduced(
    paste0(acc_outcome, " ~ difficulty * effort + (1 + difficulty + effort + difficulty:effort | sub)"),
    data = acc_beh, family = binomial(link = "logit"),
