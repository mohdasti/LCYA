...
compute_trial_auc <- function(raw, cfg) {
  ...
  raw %>%
    ...
    group_by(sub, trial_index, task, difficulty, effort,
             stimLev, isStrength, gf_trPer, auc_rel_mvc, iscorr, resp1_diff, resp1RT, conf_median,
             s2_rel, resp_rel) %>%
    summarise(
      b0  = mean(pupil[t_sq >= -0.5 & t_sq < 0], na.rm = TRUE),
      b2b = mean(pupil[t_sq >= (first(s2_rel) - 0.5) &
...
for (task_name in TASKS) {
  ...
  d <- compute_trial_auc(raw, TASK_CFG[[task_name]]) %>%
    ...
    mutate(
      ...
      auc_rel_mvc_scaled = as.numeric(scale(auc_rel_mvc)),  # actual measured force (all trials)
  ...
  # ── A_cont. Actual measured force (all trials) → Accuracy (GLMM) ──────────
  # CDT: use resp1_diff (hit rate) to match primary analysis; iscorr is unreliable
  cat("\n[A_cont] auc_rel_mvc_scaled → Accuracy (all trials)\n")
  acc_outcome  <- if (task_name == "CDT") "resp1_diff" else "iscorr"
  acc_data_cont <- if (task_name == "CDT") {
    d %>% filter(!is.na(resp1_diff))
  } else {
    d
  }
  r <- fit_reduced(
    paste0(acc_outcome, " ~ difficulty * auc_rel_mvc_scaled + (1 + difficulty + auc_rel_mvc_scaled | sub)"),
    data = acc_data_cont, family = binomial(), label = paste(task_name, "Acc~aucRelMVC")
  )
  log_append(paste(task_name, "Model A_cont: Accuracy ~ auc_rel_mvc (all trials)"), r)
  all_fe[[paste(task_name, "A_cont_Accuracy")]] <- extract_fe(
    r, task_name, "Accuracy", "GLMM", "A_cont_Behavioral"
  )

  # ── A_cont. Actual measured force (all trials) → RT (LMM) ─────────────────
  cat("\n[A_cont] auc_rel_mvc_scaled → RT (all trials)\n")
  r <- fit_reduced(
    "log_rt ~ difficulty * auc_rel_mvc_scaled + (1 + difficulty + auc_rel_mvc_scaled | sub)",
    data = d, label = paste(task_name, "RT~aucRelMVC")
  )
  log_append(paste(task_name, "Model A_cont: RT ~ auc_rel_mvc (all trials)"), r)
  all_fe[[paste(task_name, "A_cont_RT")]] <- extract_fe(
    r, task_name, "RT_log", "LMM", "A_cont_Behavioral"
  )

  # ── B_cont. Actual measured force (all trials) → Total AUC (LMM) ──────────
  cat("\n[B_cont] auc_rel_mvc_scaled → Total AUC (all trials)\n")
  b_cont_formula <- if (task_name == "CDT") {
    "Total_AUC ~ auc_rel_mvc_scaled + (1 + auc_rel_mvc_scaled | sub)"
  } else {
    "Total_AUC ~ difficulty * auc_rel_mvc_scaled + (1 + difficulty + auc_rel_mvc_scaled | sub)"
  }
  r <- fit_reduced(b_cont_formula, data = d, label = paste(task_name, "AUC~aucRelMVC"))
  log_append(paste(task_name, "Model B_cont: Total AUC ~ auc_rel_mvc (all trials)"), r)
  all_fe[[paste(task_name, "B_cont_TotalAUC")]] <- extract_fe(
    r, task_name, "Total_AUC", "LMM", "B_cont_Pupil"
  )

  # ── B_cont_cog. Actual measured force → Cognitive AUC (ADT/VDT only) ──────
  if (task_name != "CDT") {
    cat("\n[B_cont_cog] auc_rel_mvc_scaled → Cognitive AUC (ADT/VDT only)\n")
    d_cog <- d %>% filter(!is.na(Cognitive_AUC))
    b_cog_formula <- "Cognitive_AUC ~ difficulty * auc_rel_mvc_scaled + (1 + difficulty + auc_rel_mvc_scaled | sub)"
    r <- fit_reduced(b_cog_formula, data = d_cog,
                     label = paste(task_name, "CogAUC~aucRelMVC"))
    log_append(paste(task_name, "Model B_cont_cog: Cognitive AUC ~ auc_rel_mvc (all trials)"), r)
    all_fe[[paste(task_name, "B_cont_CogAUC")]] <- extract_fe(
      r, task_name, "Cognitive_AUC", "LMM", "B_cont_cog_Pupil"
    )
  }
