...
# Figure2 Clean Final Version - Updated for Aaron Seitz Comments
# 1. Remove significance brackets
# 2. Remove x-axis label from accuracy plots (keep only on reaction time plots)
# 3. Show Reaction Time in milliseconds (not log-transformed) - per Aaron's comment
...
behavioral_simple <- four_level_data %>%
    ...
    summarise(
        ...
        # Use raw RT in milliseconds (not log-transformed) - per Aaron's comment
        rt_mean_ms = mean(rt_ms, na.rm = TRUE),
        rt_se_ms = sd(rt_ms, na.rm = TRUE) / sqrt(n()),
        n_trials = n(),
        .groups = 'drop'
    ) %>%
    mutate(
...
# Updated to show RT in milliseconds (not log-transformed) - per Aaron's comment
p_rt_simple <- ggplot(behavioral_simple, aes(x = difficulty_binary, y = rt_mean_ms,
                                                fill = condition)) +
    geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
    geom_errorbar(aes(ymin = rt_mean_ms - rt_se_ms,
                      ymax = rt_mean_ms + rt_se_ms),
                  position = position_dodge(width = 0.8), width = 0.2, linewidth = 0.8) +
    facet_wrap(~task, ncol = 3) +
    scale_fill_manual(values = condition_colors) +
    labs(
        ...
        y = "Reaction Time (ms)",  # Updated to show milliseconds
        fill = "Condition"
    ) +
    theme_publication() +
    theme(axis.text.x = element_text(angle = 0, hjust = 0.5),
...
fig2_simple <- p_accuracy_simple / p_rt_simple +
...
cat("✓ Updated to show Reaction Time in milliseconds (not log-transformed) - per Aaron's comment\n")
